...
abstract contract PIController {
    function update(int256) external virtual returns (int256, int256, int256);

    function perSecondIntegralLeak() external view virtual returns (uint256);

    function elapsed() external view virtual returns (uint256);

    function getNextPiOutput(
        int error
